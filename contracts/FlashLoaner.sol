...
contract FlashLoaner {
  ...
  constructor(address _factory, address _uniRouter, address _sushiRouter) public {
    factory = _factory;
    sushiRouter = address(_sushiRouter);
  }
  ...
  function uniswapV2Call(address _sender, uint _amount0, uint _amount1, bytes calldata _data) external {
    ...
    uint amountReceived = IUniswapV2Router02(sushiRouter).swapExactTokensForTokens(amountToken,
      amountRequired, path, address(msg.sender), deadline)[1];
